...
class AutoresController extends AbstractController
{
    ...
    #[Route('/{id}', name: 'autores_show', methods: ['GET'])]
    ...
    #[Route('/{id}/edit', name: 'autores_edit', methods: ['GET','POST'])]
    ...
    #[Route('/{id}', name: 'autores_delete', methods: ['POST'])]
    public function delete(Request $request, Autores $autore): Response
    {
        if ($this->isCsrfTokenValid('delete'.$autore->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
